...
console.log(
   getSubArrayAndConvertToString(['Я', 'люблю', 'JavaScript!', 'Він', 'дуже', 'потужний'], 3),
);
...
function flattenSquareAndFilter(arr, threshold) {
   ...
   const squarArray = flatArray.map((elem) => elem ** 2);
   const filterSquarArray = squarArray.filter((elem) => elem > threshold);
   return filterSquarArray;
}
...
function getLastElementsSumAndPrint(arr, n) {
   ...
   const sum = lastElements.reduceRight((acc, elem) => acc + elem, 0);
...
function expandArrayByN(arr, n) {
   ...
   const expandedArray = arr.flatMap((value) => Array(n).fill(value));
...
function capitalizeWords(arr) {
   ...
   const capitalizedArray = arr.map((letter) => letter[0].toUpperCase() + letter.slice(1));
...
function findLastNegativeNumberIndex(arr) {
   ...
   const lastIndex = arr.findLastIndex((value) => value < 0);
   return lastIndex;
}
